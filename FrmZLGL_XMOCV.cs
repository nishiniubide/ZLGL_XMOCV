    ...
    public partial class FrmZLGL_XMOCV : FrmBase
    {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    importedData = ReadExcelWithNPOI(ofd.FileName, null);
        ...
        private void btnUpload_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            string userName = "your_basic_auth_username";
            string password = "your_basic_auth_password";
            string url = "https://xiaomi-integration-url/xxx";
            string appId = "mi_sap_po";
            string appKey = "your_app_key";

            X5ResponseMessage response = X5Client.PostData(userName, password, url, appId, appKey, package.BodyJson);
            if (response.Header.Code == "200")
            {
                Msg.ShowInformation($"上传成功！{response.Header.Desc}");
            }
            else
            {
                Msg.ShowError($"上传失败：{response.Header.Code} - {response.Header.Desc}");
            }
        ...
        private DataTable ReadExcelWithNPOI(string fileName, string sheetName, DataDimension dimension = DataDimension.LL)
        {
            try
            {
                ...
                // ================================================================
                // ✅ 修改点1：通用标题行查找策略
                // 在前5行中，查找包含 "(必填)" 关键字最多的行作为标题行
                // 此策略适用于所有维度（LL/JC/OQC/ORT/IQC/IPQC）的Excel模板
                // ================================================================
                IRow headerRow = null;
                ...
                // ================================================================
                // ✅ 修改点2：使用完整的列范围创建列，避免 headerRow.Cells 
                // 跳过空单元格导致列错位
                // ================================================================
                ...
                for (int colIndex = firstCol; colIndex < lastCol; colIndex++)
                {
                    ...
                    if (cell != null)
                    {
                        colName = cell.CellType == CellType.String
                ...
                // ================================================================
                // ✅ 修改点3：数据行读取时使用 firstCol 偏移，确保列对齐
                // ================================================================
                for (int rowIndex = headerRowIndex + 1; rowIndex <= sheet.LastRowNum; rowIndex++)
                {
                    ...
                    {
                        // ✅ 使用 firstCol + colIndex 获取实际列位置的单元格
